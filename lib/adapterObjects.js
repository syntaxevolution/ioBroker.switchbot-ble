...
async function createBotObjects(adapter, data) {
    adapter.log.info(`[createBotObjects] Found model: ${data.serviceData.model} for device ${data.address}`);
    ...
    } else if (data.serviceData.model === 'o') {
        // SwitchBot Lock Pro
        await adapter.createChannelNotExists(data.address + '.control');
        await adapter.createObjectNotExists(
            data.address + '.control.lockProCommand',
            'Send command to Lock Pro (lock, unlock, unlatch)',
            'string',
            'text',
            true,
            '',
